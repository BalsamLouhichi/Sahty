...
use App\Service\EvenementExperienceDesignService;
use App\Service\EvenementOperationalInsightsService;
...
class EvenementController extends AbstractController
{
    ...
    private EvenementOperationalInsightsService $operationalInsightsService;
    private EvenementSeriesInsightsAIService $evenementSeriesInsightsAIService;
    ...
    public function __construct(
        ...
        EvenementOperationalInsightsService $operationalInsightsService,
    ...
    )
    {
        ...
        $this->evenementSeriesInsightsAIService = $evenementSeriesInsightsAIService;
    ...
    public function edit(Request $request, Evenement $evenement, EntityManagerInterface $em): Response
    {
        ...
        $form = $this->createForm(EvenementType::class, $evenement, [
            ...
            'is_admin' => $isAdmin,
            'is_edit' => true,
            'series_candidates' => $this->buildSeriesCandidateChoices($evenement->getId()),
            'series_edition_number' => $this->extractEditionNumberFromTitle((string) $evenement->getTitre()),
        ]);
    ...
    #[Route('/api/recommandations-lieux', name: 'api_lieux_recommandes', methods: ['GET'])]
    public function apiLieuxRecommandes(Request $request): JsonResponse
    {
        $ville = trim((string) $request->query->get('ville', ''));
        $type = trim((string) $request->query->get('type', ''));
        $mode = trim((string) $request->query->get('mode', ''));
        $capacity = $request->query->getInt('capacity', 0);
        ...
        $durationHours = $request->query->get('duration_hours');

        if ($ville === '' || $type === '') {
            return $this->json([
                'success' => false,
                'message' => 'Renseignez la ville et le type pour lancer la recommandation.',
                'lieux' => [],
            ], 400);
        }
        ...
        $result = $this->evenementVenueRecommendationService->recommend(
            $ville,
            $type,
    ...
    #[Route('/api/plan-ressources', name: 'api_plan_ressources', methods: ['GET'])]
    public function apiPlanRessources(Request $request): JsonResponse
    {
        $type = trim((string) $request->query->get('type', ''));
        $mode = trim((string) $request->query->get('mode', ''));
        $capacity = $request->query->getInt('capacity', 0);

        if ($type === '' || $mode === '' || $capacity <= 0) {
            return $this->json([
                'success' => false,
                'message' => 'Renseignez le type, le mode et une capacite valide pour lancer l analyse.',
            ], 400);
        }

        $plan = $this->evenementResourcePlanningService->buildPlan($type, $mode, $capacity);
    ...
    public function demandesEvenements(
        ...
        $resourcePlansByDemande = [];

        foreach ($demandes as $demande) {
            $resourcePlansByDemande[$demande->getId()] = $this->evenementResourcePlanningService->buildPlan(
                (string) $demande->getType(),
                (string) $demande->getMode(),
                $demande->getPlacesMax()
            );
        }
        ...
        return $this->render('evenement/admin_demandes_evenements.html.twig', [
            ...
            'recherche' => $recherche,
            'type' => $type,
            'resource_plans_by_demande' => $resourcePlansByDemande,
        ]);
    }
    ...
    private function appendSchedulingConflictErrors(Evenement $evenement, array &$errors, ?int $excludeEventId): void
    {
        $conflicts = $this->evenementRepository->findSchedulingConflicts($evenement, $excludeEventId);
        ...
        $errors[] = sprintf(
            'Conflit de planification : cet evenement chevauche "%s" (%s - %s) au meme lieu. Merci de choisir un autre horaire ou un autre lieu.',
            (string) $firstConflict->getTitre(),
            $firstConflict->getDateDebut()?->format('d/m/Y H:i'),
            $firstConflict->getDateFin()?->format('d/m/Y H:i')
        );
    }

    private function appendOperationalValidationErrors(Evenement $evenement, array &$errors): void
    {
        $insights = $this->operationalInsightsService->analyze($evenement);

        if (($insights['location_valid'] ?? true) === false && !empty($insights['location_message'])) {
            $errors[] = (string) $insights['location_message'];
        }
    }
    ...
    {
        $choices = [];
        $events = $this->evenementRepository->findBy([], ['dateDebut' => 'DESC']);

        foreach ($events as $event) {
            if ($excludeEventId !== null && $event->getId() === $excludeEventId) {
                continue;
            }
            ...
            $label = sprintf(
                ...
                $event->getDateDebut() ? $event->getDateDebut()->format('d/m/Y') : 'Date non definie'
            );
            $choices[$label] = (string) $event->getId();
        }
    ...
    {
        if (!$form->has('isEdition') || !$form->has('editionNumero')) {
            return;
        }

        $isEdition = (bool) $form->get('isEdition')->getData();
        if (!$isEdition) {
            return;
        }

        $editionNumero = $form->get('editionNumero')->getData();
        if (!is_numeric($editionNumero) || (int) $editionNumero <= 0) {
            return;
        }

        $editionNumero = (int) $editionNumero;
        $titre = trim((string) $evenement->getTitre());
        if ($titre === '') {
            return;
        }

        $baseTitle = preg_replace('/\s*[-–—]\s*edition\s*\d+\s*$/iu', '', $titre);
        $baseTitle = trim((string) ($baseTitle ?? $titre));
        $evenement->setTitre(sprintf('%s - Edition %d', $baseTitle, $editionNumero));
    }

    private function extractEditionNumberFromTitle(string $title): ?int
    {
        if (preg_match('/(?:edition|ed\.?)\s*#?\s*(\d+)/iu', $title, $matches) === 1) {
